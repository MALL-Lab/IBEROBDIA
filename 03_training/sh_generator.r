...
for (i in 1:length(input.paths)) {
  for (j in 1:length(input.algs)) {
    ...
    system(paste('sbatch ', '/mnt/netapp2/Store_uni/home/ulc/co/dfe/git/IBEROBDIA/03_training/exec/', substr(input.algs[j], 1, nchar(input.algs[j])-7) ,substr(input.paths[i], 1, nchar(input.paths[i])), '.sh', sep = ''))
