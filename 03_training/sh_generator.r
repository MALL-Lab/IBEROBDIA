...
for (i in 1:length(input.paths)) {
  for (j in 1:length(input.algs)) {
    ...
    cat(paste("#SBATCH", paste0("--error=", wd, outslurm.dir, "error-", substr(input.algs[j], 1, nchar(input.algs[j]) - 7),
               substr(input.paths[i], 1, nchar(input.paths[i])-4), ".txt")))
    cat(paste("#SBATCH", paste0("--output=", wd, outslurm.dir, "output-", substr(input.algs[j], 1, nchar(input.algs[j]) - 7),
               substr(input.paths[i], 1, nchar(input.paths[i])-4),".txt")))
    ...
    cat(paste('Rscript /mnt/netapp2/Store_uni/home/ulc/co/dfe/projects/IBEROBDIA/03_training/models/', input.algs[j],  " $data", " $name", sep=''))
    
    sink(file = NULL)
    
    system(paste('sbatch ', '/mnt/netapp2/Store_uni/home/ulc/co/dfe/projects/IBEROBDIA/03_training/exec/', substr(input.algs[j], 1, nchar(input.algs[j])-7) ,substr(input.paths[i], 1, nchar(input.paths[i])), '.sh', sep = ''))
